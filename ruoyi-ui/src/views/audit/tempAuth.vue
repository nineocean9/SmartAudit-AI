  ...
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="authList">
      ...
      <el-table-column label="授权类型" prop="authType" width="100">
        <template #default="scope"><el-tag>{{ getAuthTypeLabel(scope.row.authType) }}</el-tag></template>
      </el-table-column>
    ...
    <el-dialog title="新增授权" v-model="dialogVisible" width="500px">
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="授权类型">
          <el-select v-model="form.authType"><el-option label="中介审计" value="intermediary"/><el-option label="临时授权" value="temp"/></el-select>
        </el-form-item>
...
const form = ref({ authType: 'intermediary' })
...
function getAuthTypeLabel(authType) {
  return authType === 'intermediary' || authType === 'outsource' ? '中介审计' : '临时授权'
}
function handleAdd() { form.value = { authType: 'intermediary' }; dialogVisible.value = true }
function submitForm() {

...
-- ----------------------------
-- 其他、存储过程find_in_set 视图view_self_table,view_self_table_columns
-- ----------------------------
CREATE OR REPLACE FUNCTION find_in_set(STR BIGINT, STRS VARCHAR)
RETURNS BOOLEAN AS $$
DECLARE
  POS INTEGER;
BEGIN
  SELECT POSITION(',' || STR::VARCHAR || ',' IN ',' || STRS || ',') INTO POS;
  RETURN POS > 0;
END;
$$ LANGUAGE PLPGSQL;

...
CREATE INDEX idx_pd_project ON project_document(project_id);
CREATE INDEX idx_pd_plan ON project_document(plan_id);
CREATE INDEX idx_pd_type ON project_document(project_id, doc_type);
...
CREATE INDEX idx_tw_session ON temporary_workspace(session_id);
CREATE INDEX idx_tw_expire ON temporary_workspace(expire_time);
...
SELECT 1089, '项目文档', 1062, 15, 'projectDoc', 'audit/projectLib', '', '', 1, 0, 'C', '0', '0', 'audit:projectDoc:view', 'documentation', 'admin', now(), '', NULL, '项目文档管理'
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE menu_id = 1089);

INSERT INTO sys_menu (menu_id, menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
SELECT 1090, '项目文档上传', 1089, 1, '', '', '', '', 1, 0, 'F', '0', '0', 'audit:projectDoc:upload', '#', 'admin', now(), '', NULL, ''
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE menu_id = 1090);

INSERT INTO sys_menu (menu_id, menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
SELECT 1091, '项目文档删除', 1089, 2, '', '', '', '', 1, 0, 'F', '0', '0', 'audit:projectDoc:remove', '#', 'admin', now(), '', NULL, ''
WHERE NOT EXISTS (SELECT 1 FROM sys_menu WHERE menu_id = 1091);

-- 数据分析 (menu_id=1092)
INSERT INTO sys_menu (menu_id, menu_name, parent_id, order_num, path, component, query, route_name, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
SELECT 1092, '数据分析', 1062, 16, 'dataAnalyze', 'ai/dataAnalyze', '', '', 1, 0, 'C', '0', '0', 'ai:data:analyze', 'chart', 'admin', now(), '', NULL, '数据分析页面'
